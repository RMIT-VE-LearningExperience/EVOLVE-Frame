try{
var data=Newtonsoft.Json.Linq.JArray.Parse(System.IO.File.ReadAllText(@"C:\Users\Stormtrooper\Git\EVOLVE - Frame\output\revit\roof_members_manifest.json"));
...
using(var tx=new Transaction(document,"Leichhardt - supplier roof truss members")){tx.Start();
 ...
 foreach(var row in data.Where(r=>!existing.Contains((string)r["mark"])).Take(150)){
